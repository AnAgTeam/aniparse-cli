...
int parse_serialized_record(ParserStore& store, RequestorContext& ctx,
	...
	const std::string category(category_value->as_string().c_str());
	...
	if (category == "manga") {
		auto getter = coro::sync_wait(parser->mangas_getter()->from_serialized(std::move(data)));
		if (!getter) {
	...
	if (category == "anime") {
		auto getter = coro::sync_wait(parser->animes_getter()->from_serialized(std::move(data)));
		if (!getter) {
			std::println(stderr, "{}: from_serialized failed: {}", program, getter.error().message);
			return Runtime;
		}
	...
	if (category == "images") {
		auto getter = coro::sync_wait(parser->images_getter()->from_serialized(std::move(data)));
		if (!getter) {
			std::println(stderr, "{}: from_serialized failed: {}", program, getter.error().message);
			return Runtime;
		}
...
int episodes(std::span<const std::string_view> args, bool json) {
	...
	if (!url) {
		std::println(stderr, "{}: episodes needs an anime <url>", program);
		return Usage;
	}
	const std::optional<GetFilters> filters = parse_filters(args);
	...
	std::unique_ptr<AnimeGetter> getter = std::move(*getter_result);
	const std::string source = route->parser->info().name;
	...
	if (!track && track_first) {
		...
		if (json) {
			...
			std::println("{}", boost::json::serialize(out));
		} else {
